  ...
  const getData = () => {
    ...
    if (search.length == 0) {
      ...
      fetch(uri)
        ...
        .then((res_json) => {
          let listBillsByMonth = res_json.data.filter(
            (item) => item.dateRent.substring(0, 7) == search
          );
          setList([...listBillsByMonth]);
          let tong = 0;
          listBillsByMonth.forEach((item)=>{
...
export default DoanhThuThangChiTiet;
